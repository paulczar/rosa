...
func run(cmd *cobra.Command, _ []string) {
			...
			if args.debug {
				...
				for _, envVar := range allEnvVars {
					if len(envVar) >= 16 && envVar[:16] == "OPENAI_API_KEY=" {
						found = true
						if len(envVar) == 16 {
							fmt.Fprintf(os.Stderr, "Debug: OPENAI_API_KEY exists but is empty\n")
						} else {
							fmt.Fprintf(os.Stderr, "Debug: OPENAI_API_KEY exists with length %d (first 10 chars: %s...)\n", len(envVar)-16, envVar[16:26])
						}
						break
					}
				}
				if !found {
					fmt.Fprintf(os.Stderr, "Debug: OPENAI_API_KEY not found in environment variables\n")
					fmt.Fprintf(os.Stderr, "Debug: Total env vars: %d\n", len(allEnvVars))
				}
			}

			fmt.Fprintf(os.Stderr, "Error: API key required. Set OPENAI_API_KEY environment variable or use --api-key flag.\n")
			os.Exit(1)
		}
		apiKey = envKey
	}
	...
	if apiKey == "" {
		fmt.Fprintf(os.Stderr, "Error: API key is empty. Please provide a valid API key.\n")
		os.Exit(1)
	}
	...
	if apiKey == "" {
		fmt.Fprintf(os.Stderr, "Error: API key is empty after validation. This should not happen.\n")
		os.Exit(1)
	}
	...
	if args.systemMessageFile != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error reading system message file: %v\n", err)
			os.Exit(1)
		}
